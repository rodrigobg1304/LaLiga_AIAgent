"""
Script de entrenamiento GENÉRICO para modelos Over/Under CORNERS
Arquitectura: 10 Random Forest independientes con calibración isotónica
...
from football_agent.db import get_matches_with_corners
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Entrenar modelos Over/Under para Corners'
    )
    parser.add_argument(
        '--leagues',
        nargs='+',
...
def get_all_matches(league_id: str, years: list) -> pd.DataFrame:
    """Obtiene todos los partidos con corners."""
    rows = get_matches_with_corners(league_id, years)
    ...
    print(f"   Distribución de corners:")
    print(f"   Home corners: mean={df['home_corners'].mean():.1f}, max={df['home_corners'].max()}")
    print(f"   Away corners: mean={df['away_corners'].mean():.1f}, max={df['away_corners'].max()}")

    return df


def prepare_dataset_corners(df: pd.DataFrame, league_id: str) -> tuple:
    """
    Prepara dataset para corners con arquitectura is_home.

    Cada partido genera 2 rows:
    - Row 1: features + is_home=1 → target: home_corners
    - Row 2: features + is_home=0 → target: away_corners
    """
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="   Procesando"):
        try:
            ...
            # Row 1: Home corners (is_home=1)
            features_home = np.append(base_features, [1])
            X_list.append(features_home)
            y_corners_list.append(row['home_corners'])

            # Row 2: Away corners (is_home=0)
            features_away = np.append(base_features, [0])
            X_list.append(features_away)
            y_corners_list.append(row['away_corners'])

        except Exception as e:
            continue

    X = np.vstack(X_list)
    y_corners = np.array(y_corners_list)

    print(f"✅ Features: {X.shape} | Total examples: {len(y_corners)}")
    ...
    for threshold in CORNERS_THRESHOLDS:
        ...
        y_dict[col_name] = (y_corners > threshold).astype(int)

    return X, y_dict, y_corners


def train_model_for_threshold(X_train, y_train, X_test, y_test, threshold: float):
    """Entrena modelo RF para un threshold de corners."""
...
def train_league(league_key: str):
    """Entrena modelos Over/Under corners para una liga."""
    ...
    print("\n" + "=" * 60)
    print(f"🚀 ENTRENAMIENTO CORNERS - {league_name}")
    print("=" * 60)
    print(f"Liga ID: {league_id}")
    print(f"Train: {TRAIN_SEASONS}")
    print(f"Test:  {TEST_SEASONS}")
    print(f"Thresholds: {CORNERS_THRESHOLDS}")

    # Directorio de salida
    output_dir = Path(MODEL_BASE_DIR) / "models" / "over_under" / "corners" / "production" / league_slug
    ...
    X_train, y_train_dict, corners_train = prepare_dataset_corners(df_train, league_id)
    X_test, y_test_dict, corners_test = prepare_dataset_corners(df_test, league_id)
    ...
    for threshold in CORNERS_THRESHOLDS:
        ...
        # Guardar modelo
        model_filename = f"model_{col_name}_corners_{league_slug}.pkl"
    ...
    # 4. Guardar métricas
    metrics_path = output_dir / f"metrics_corners_{league_slug}.json"
    ...
    # 5. Resumen
    print("\n" + "=" * 60)
    print(f"✅ {league_name} CORNERS - ENTRENAMIENTO COMPLETADO")
    print("=" * 60)
    print("\nResumen de modelos:")
    for threshold in CORNERS_THRESHOLDS:
        col_name = f'over_{str(threshold).replace(".", "_")}'
...
def main():
    ...
    print("\n" + "=" * 60)
    print("🎯 ENTRENAMIENTO CORNERS - SCRIPT GENÉRICO")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("✅ TODOS LOS ENTRENAMIENTOS CORNERS COMPLETADOS")
    print("=" * 60)
